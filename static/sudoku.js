...
var sudoku = new Array( 0,0,2,4,0,8,0,0,0,
                        0,0,7,0,5,0,0,0,0,
                        0,4,1,0,0,2,9,0,0,
                        0,0,0,3,0,6,0,0,4,
                        0,6,0,0,1,0,0,0,0,
...
function showSudoku(sudoku,i) {
	...
	for (var i=0; i<=8; i++) {
		...
		for (var j=0; j<=8; j++) {
            var cellId = i*9+j
            sudokuText+="<td id='sdk"+cellId+"' class='js-open-modal'>";
            var val = sudoku[i*9+j];
            if (val!=0)
	...
	sudokuText+="</table>";
	document.getElementById("sudoku").innerHTML = sudokuText;
}

function updateSudokuCell(predictedCellID, value){
    var position = returnIDfromCellID(predictedCellID);
    var val = parseInt(value);

    $('#'+predictedCellID).text(value);

    if (isPossibleNumber(position,val,sudoku))
        $('#'+predictedCellID).removeClass("wrongCell");
    else
        $('#'+predictedCellID).addClass("wrongCell");
    
    sudoku[position] = val;
    //showSudoku(sudoku);
}

function returnIDfromCellID(cellID){
    return parseInt(cellID.replace("sdk", ""));
}
